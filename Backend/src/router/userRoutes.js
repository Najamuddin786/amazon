...
router.post('/buy', async (req, res) => {
  let data = req.body;
  try {
      let products = await productModel.findOne({ _id: data.id });
      let user = await userSingupModel.findOne({ number: data.number });
      const currentTime = moment().tz('Asia/Kolkata').format('YYYY-MM-DDTHH:mm:ss');

      if (products) {
          if (user && user.recharge && user.recharge.length > 0) {
              if (products.price <= user.recharge[0].amount) {
                  let remainingAmount = user.recharge[0].amount - products.price;
                  // user.recharge[0]['amount'] =remainingAmount;
                  user.recharge.push({'amount':remainingAmount})
                  user.product.push(products)

                  // Save the updated user document
                  await user.save();

                  console.log("Product Buy Done");
                  console.log("Remaining Amount:", remainingAmount);

                  res.json({ message: "Product purchased successfully", product: products });
              } else {
                  console.log("Product not Buy Done - insufficient funds");
                  res.status(400).json({ message: "Insufficient funds" });
              }
          } else {
              console.log("User not found or no recharge info");
              res.status(400).json({ message: "User not found or no recharge information available" });
          }
      } else {
